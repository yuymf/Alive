...
- **Flow engine** (`flow-engine.ts`) — State machine managing **flow** (immersion: intensity - resistance > 2.0, reduces vitality drain ×0.7, skips LLM call) and **drift** (slacking: low vitality + no strong intents + low stress, also skips LLM). Both states use template-generated diary entries instead of LLM calls. Flow exits on gap closure, low vitality, schedule block, or random interrupt; drift exits on strong intent, new event, or vitality recovery.
- **Vitality engine** (`vitality-engine.ts`) — 0-100 resource that drains per tick (with optional flow modifier) and per action, gates posting (must be >30), with emergency recovery after 3 consecutive low days.
