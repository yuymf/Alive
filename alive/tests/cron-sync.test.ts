...
import { findCronJobByName, editCronExpression, buildCronExpression, syncCronSchedule } from '../scripts/lifecycle/cron-sync';
...
describe('cron-sync', () => {
  ...
  describe('syncCronSchedule', () => {
    it('should find each job by name and edit its cron expression', () => {
      ...
      const result = syncCronSchedule(schedule, 'minase');
    ...
    it('should list jobs only once (not per job)', () => {
      ...
      syncCronSchedule(schedule, 'minase');
