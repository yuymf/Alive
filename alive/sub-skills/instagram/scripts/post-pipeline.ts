...
import { generateImageSet, GenerateSetResult } from '../../platform/generate-image/scripts/provider';
import { uploadToImgURL } from '../../platform/gallery/scripts/imgurl-upload';
...
// Local type alias for pending reply scheduling
...
interface PendingEngagement { pending_replies: PendingReply[] }

/** Schedule a comment check for a newly posted media (local implementation). */
function scheduleCommentCheck(entry: Omit<PendingReply, 'replied_comment_ids'>): void {
...
/** Refresh inspiration data if content-browse sub-skill supports it. */
async function refreshInspiration(): Promise<void> {
  // content-browse's refreshInspiration is available only via SubSkillContext.
  // In standalone pipeline mode, inspiration refresh is handled by heartbeat-tick.
  console.log('[pipeline] Inspiration refresh delegated to heartbeat-tick.');
}
