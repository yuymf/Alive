...
- **被调优对象**：`eval/tunable/prompts/**` 目录下的 prompt 文件（其他代码不允许动）
...
所有接入的 ops 模块先调 `readTunablePrompt(relativePath)`；有 override 则用 override，没有就走模块内置 fallback prompt。这样 `eval/tunable/prompts/ops/*.md` 的每一次修改，都会被下一次 fixture run 用到。
...
**只允许**改 `eval/tunable/prompts/**` 里的 `.md` 文件。

不允许碰：

- `alive/scripts/**`、`alive/tests/**`、`e2e/**`：实现 + 评分器
- `eval/program.md`、`eval/README.md`：治理文件
- 任何 `*.ts` / `*.json`：结构化配置 / 代码
...
1. 没有 `eval/tunable/prompts/` 安装时（生产环境），系统行为与原先完全一致。
...
2. `<repoRoot>/eval/tunable/prompts/<relative>` —— 开发时的 repo 路径

所以在 repo 里改 `eval/tunable/prompts/ops/*.md`，直接跑 `pnpm autoresearch:fixture` 就会生效。
