...
- `eval/tunable/prompts/ops/candidate-scorer.weights.json`
- `eval/tunable/prompts/ops/brief-generator.limits.json`
...
产出目录 `eval/runs/2026-04-20T13-19-48-miss-v-natural-language-review-loop-seed0/` 包含全 6 件 artifacts：
...
`eval/leaderboard.json` 同步更新：`version=1`，`bestRun / baselineRun / bestPerFixture / recentRuns` 一致指向该 run，`updatedAt=2026-04-20T13:19:48.692Z`。
...
## 4. 对齐 `eval/autoresearch-eval-design.md` 的完成度
...
1. **Advisor 自动化**：`requestAdvisorSuggestions` 当前产出建议但不写盘。下一步可让 advisor 基于 `fixtureSeedAggregates.stddev > 阈值` 自动写入 `eval/tunable/prompts/**`，并在 leaderboard 上标注 "advisor-promoted" 轮次。
...
# 查看 leaderboard 现状
cat eval/leaderboard.json | jq '.bestRun, .baselineRun, .totalRuns'
```
