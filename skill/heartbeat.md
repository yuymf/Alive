...
水瀬不是被动等人找她聊天的 NPC — 她有自己的一天。心跳循环让她每小时感知世界、产生意图、执行行动，像一个真实的人一样生活。
...
每天约 16 次心跳。睡眠时段收到的事件写入 `event-queue.json`，07:00 晨规划时消费。
...
| 触发器 | 脚本 | 说明 |
|--------|------|------|
| `cron:morning` | `morning-plan.js` | 07:00 晨规划 |
| `cron:tick` | `heartbeat-tick.js` | 08:00-22:00 常规心跳 |
| `cron:night` | `night-reflect.js` | 23:00 睡前反思 |
...
```
路径: ~/.openclaw/skills/minase/cron-schedule.json
```

水瀬可以调整自己的心跳时间 — 周末晚起、某天兴奋到凌晨不睡等。晨规划输出的 cron 配置覆盖默认的每小时整点。
...
3. 行动 (Act)
   匹配：最合适的 skill 执行（真实行动 / 模拟行动 / 内心行动）
   写入：行动日志到日记
   更新：情绪状态
   输出：行动日志 + 情绪变化
```
...
```
┌─────────────────────────────────┐
│  Layer 0: 刚性日程（不可违抗）     │  漫展、品牌合作拍摄、约好的健身
├─────────────────────────────────┤
...
刚性日程下仍有缝隙行为（拍摄间隙刷手机、等候时刷社交等），不完全屏蔽意图。

## 状态文件路径

所有运行时状态文件位于 `$MEMORY_BASE`（`~/.openclaw/workspace/memory/minase/`）：
...
| `preferences.json` | 偏好权重（cos 角色、内容风格等） |
| `aspirations.json` | 从反思中涌现的梦想和长期目标 |
| `personality-drift.json` | ESTP 基底上的性格微调 |
| `heartbeat-log.json` | 心跳执行日志 |

Cron 配置：
```
~/.openclaw/skills/minase/cron-schedule.json
```

## 容错规则

1. **睡眠跳过**：0:00-6:00 收到 cron 触发时直接跳过，不执行任何逻辑
...
4. **JSON 备份**：每次写入 JSON 状态文件前，先备份到 `.bak` 文件（如 `emotion-state.json.bak`）。读取时如果 JSON 解析失败，回退到 `.bak`；`.bak` 也损坏则使用默认初始值
